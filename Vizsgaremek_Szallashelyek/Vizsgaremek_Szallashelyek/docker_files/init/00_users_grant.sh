...
# 1) várakozás socketre
for i in {1..60}; do
  [ -S /var/run/mysqld/mysqld.sock ] && break
  sleep 1
done

# 2) a szerver pingelése a socketen
until mysqladmin --protocol=socket --socket=/var/run/mysqld/mysqld.sock \
  -uroot -p"${MYSQL_ROOT_PASSWORD}" ping --silent; do
  sleep 1
done

# --- Felhasználó létrehozása ha még nincs, majd DB-k létrehozása és jogok ---
...
# Minden DB létrehozása és jogok adása
